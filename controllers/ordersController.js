...
module.exports = app => {
    let ordersService = app.services.ordersService;
    let validator = app.validators.ordersValidator;
    let searchValidator = app.validators.searchOrdersValidator;
 ...
 */
    ...
    function fetchOrders (req, res, next) {
        validator.validateFetchOrderParams(req.query).then(params => {
            return ordersService.fetchOrders(params);
        }).then(response => {
            res.send(response);
        }).catch(err => next(err));
    };

    function fetchGraphOrders (req, res, next) {
        validator.validateFetchOrderParams(req.query).then(params => {
            return ordersService.fetchGraphOrders(params);
        }).then(response => {
            res.send(response);
        }).catch(err => next(err));
    };
    ...
    function fetchAllOrders (req, res, next) {
        validator.validateFetchOrderParams(req.query).then(params => {
            return ordersService.fetchAllOrders(params);
        }).then(response => {
            res.send(response);
        }).catch(err => next(err));
    };

    function searchAllOrders (req, res, next) {
        searchValidator.validateSearchOrderParams(req.query).then(params => {
            return ordersService.fetchSearchOrderResults(params);
        }).then(response => {
            res.send(response);
        }).catch(err => next(err));
    };
